  ...
  useEffect(() => {
    ...
    axiosInstance.get('/feicom/api/exercices/')
      .then(res => {
        const options = res.data.map(item => ({
          value: item.id,
          label: `${item.id} - ${item.annee}`
        }));
        setexerciceOptions(options);
      })
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="mb-2">
        ...
        >
          <option value="">Sélectionnez un exercise</option>
...
const ProjetTable = ({ filters = {} }) => {
  ...
  const tableColumns = [
    ...
    { name:"Exercise", selector: row => row.exercice},
    {
      name: 'Actions',
      cell: row => (
        <div className='d-flex gap-1'>
          ...
          <Btn attrBtn={{ 
            ...
            size: 'sm', 
            className: 'btn-sm py-1 px-2', 
            onClick: () =>navigate(`${process.env.PUBLIC_URL}/feicom/projets/${row.id}/detail`) 
          }}>
